...
function MinimapButton:Load()
    ...
    self:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("XD Preservation Evoker")
        GameTooltip:Show()
        end)
end
...
--Draw allies as healer
Draw(function(draw)
    if select(2,IsInInstance()) == "arena" then
        if project.settings.healerDrawingsAll then
            awful.group.loop(function(friend)
                local px, py, pz = player.position()
                local distance = friend.distanceliteral
                local x,y,z = friend.position()
                local los = friend.los
...
Draw(function(draw)
    if select(2,IsInInstance()) == "arena" then
        if project.settings.healerDrawingsAll then
            if enemyHealer.exists and not enemyHealer.dead then
                local px, py, pz = player.position()
                local distance = enemyHealer.distanceliteral
                local x,y,z = enemyHealer.position()
                local los = enemyHealer.los
                if x and y and z and px and py and pz then
                    -- Out of Range
                    if distance > 25 then
                        draw:SetColor(255, 0, 0, 175)
...
togglePVP:Checkbox({
  text = "Auto WingBuffet and TailSwipe                                   ",
  var = "knockbacks",
  default = true,
})
togglePVP:Checkbox({
...
textAndDrawingsToggle:Checkbox({
  text = "All Healer Drawings                         ",
  var = "healerDrawingsAll",
  default = true,
})
...
local dispelOptional = {
    ...
    { label = awful.textureEscape(2823, 22).."Deadly Poison", value = 2823},
    { label = awful.textureEscape(200947, 22).."High Winds", value = 200947},
    { label = awful.textureEscape(15487, 22).."Silence", value = 15487},
    { label = awful.textureEscape(34914, 22).."Vampiric Touch", value = 34914,},
    { label = awful.textureEscape(217832, 22).."Imprison", value = 217832,},
    { label = awful.textureEscape(410201, 22).."Searing Glare", value = 410201,}, 
}
local dispelDefault = {
...
local dispelCurseOptional = {
    { label = awful.textureEscape(51514, 22).."Hex", value = "Hex", tvalue = {51514, 211015, 211010,211004,210873,269352,277778,277784,} },
    { label = awful.textureEscape(80240, 22).."Havoc", value = 80240},
    { label = awful.textureEscape(335467, 22).."Devouring Plague", value = 335467},
    { label = awful.textureEscape(1943, 22).."Rupture", value = 1943},
    { label = awful.textureEscape(703, 22).."Garrote", value = 703},
    { label = awful.textureEscape(1079, 22).."rip", value = 1079},
    { label = awful.textureEscape(360194, 22).."Deathmark", value = 360194},
}
local dispelCurseDefault = {
    "Hex",
    80240,
    335467,
    1943,
...
local dispelsCurse = gui:Tab("DISPEL CURSE/BLEED/POISON")
dispelsCurse:Dropdown({
	var = "dispelsCurseList",
	multi = true,
	tooltip = "Choose spells you want to dispel",
    options = dispelCurseOptional,
    default = dispelCurseDefault,
	placeholder = "Select Dispels",
	header = "Spells to Dispel:",
})
