...
if awful.player.spec == "Arcane" then awful.print(awful.player.spec) end
if awful.player.spec ~= "Arcane" and awful.player.spec ~= "奥数" then return end
...
project.mage = {}
project.mage.arcane = awful.Actor:New({ spec = 1, class = "mage" })
local arcane = project.mage.arcane
arcane.ACTOR_TICKRATE = 0.02
...
awful.Populate({
    ...
    --## PURGES ##--
    kleptomania = NewSpell(198100, { targeted = true, ignoreMoving = true, ignoreFacing = true}),
    ...
    prismaticBarrier = NewSpell(235450, { beneficial = true, ignoreMoving = true, ignoreFacing = true }),
        ...
        awful.enemies.loop(function(enemy)
            if enemy.player then
                ...
                if enemy.buffFrom(enemyReflect) then -- remove reflect
                    if fireBlast.cd == 0 then
                        fireBlast:Cast(enemy)
                    end
                end
                if arcaneBarrage:Castable(enemy) and unit.hp > 15 and enemy.hp < 15 and not enemy.buffFrom(totalimmunity) and not enemy.debuffFrom(totalimmunity) and not enemy.bcc and enemy.los and not enemy.buffFrom(enemyReflect) then
                    arcaneBarrage:Cast(enemy)
                    return true
                end
            end
        end)
...
arcane:Init(function()
    --mass invis freeze healer
    if player.buff(massInvis.id) then
        if project.settings.ringOfFrostToggle and ringOfFrost:Castable(enemyHealer) then
            ringOfFrost:SmartAoE(enemyHealer, { movePredTime = 0.5 })
        end
    end
    ...
    --prismaticBarrier
    if not player.buff(prismaticBarrier.id) then
        prismaticBarrier:Cast() 
    end
    ...
    if player.combat and not player.buff(342246) and not player.buff(32727) then -- 342246 alterTime, 32727 arena prep
        ...
        if player.hp <= project.settings.defensivesMassInvis and massInvis.cd == 0 then
            massInvis:Cast()
        end
    end
    ...
    if TargetCheck(target) and not player.buff(110960) and not player.buff(32612) and not player.buff(66) then --greater invis + normal invis(66+32612)
            ...
            if not enemy.dead then
                if enemy.player then
                    if enemy.speed > 25 then
                        if iceNova:Castable(enemy) and not enemy.buffFrom(enemyReflect) and not enemy.buffFrom(cantRootOrKnock) then
                            iceNova:Cast(enemy)
                        end
                    else
        ...
        --arcaneSurge
        if arcaneSurge:Castable(target) then
            arcaneSurge:Cast(target) 
        end

        --touchOfTheMagi
        if touchOfTheMagi:Castable(target) and player.casting and player.castID == arcaneSurge.id or arcaneSurge.cd > 10 then
            touchOfTheMagi:Cast(target) 
        end

        --arcaneMissles
        if arcaneMissles:Castable(target) and (player.buff(263725) or player.buff(276743)) then --263725+276743 clearcasting
            arcaneMissles:Cast(target) 
        end

        --presenceOfMind
        if presenceOfMind:Castable() then
            presenceOfMind:Cast() 
        end

        --arcaneOrb
        if arcaneOrb:Castable() and target.los and player.arcanecharges <= 2 and target.distance < 40 then
            player.face(target)
            arcaneOrb:Cast() 
        end

        --arcaneBlast
        if arcaneBlast:Castable(target) and player.buff(netherPrecision.id) then
            arcaneBlast:Cast(target) 
        end

        --arcaneBarrage
        if arcaneBarrage:Castable(target) and player.arcanecharges == 4 and player.buffStacks(384455) == 10 then -- 384455 Arcane harmony
            arcaneBarrage:Cast(target) 
        end
        ...
        --radiantSpark
        if radiantSpark:Castable(target) then
            radiantSpark:Cast(target) 
        end

        --shiftingPower
        if arcaneSurge.cd > 10 and touchOfTheMagi.cd > 10 and ringOfFire.cd > 10 and not player.moving and target.hp > 50 then
        ...
        if target.hp >= 20 then
            if project.settings.spellStealButton and not WasCasting[spellSteal.id] then
                if player.manaPct > 20 then
                    awful.enemies.loop(function(enemy)  
                        if TargetCheck(enemy) then
                            if spellSteal:Castable(enemy) or kleptomania:Castable(enemy) then
                                if enemy.distance < spellSteal.range or enemy.distance < kleptomania.range then
                                    if enemy.purgeCount > 0 then
                                        spellSteal:Cast(enemy)
                                        kleptomania:Cast(enemy)
                                    end
                                end
                            end
                        end
        ...
        --arcaneBarrage
        if arcaneBarrage:Castable(target) and player.moving or player.manaPct < 10 then
            arcaneBarrage:Cast(target) 
        end

        --arcaneMissles
        if arcaneMissles:Castable(target) and not player.buff(netherPrecision.id) then
            arcaneMissles:Cast(target) 
        end
    end
end)
