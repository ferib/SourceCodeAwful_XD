...
if awful.player.spec == "Fire" then awful.print(awful.player.spec) end
if awful.player.spec ~= "Fire" and awful.player.spec ~= "火焰" then return end
...
project.mage = {}
project.mage.fire = awful.Actor:New({ spec = 2, class = "mage" })
local fire = project.mage.fire
fire.ACTOR_TICKRATE = 0.02
...
awful.Populate({
    ...
    fireBlast = NewSpell(108853, { damage = "magic", targeted = true, ignoreMoving = true }),
    ...
    arcaneSurge = NewSpell(365350, { damage = "magic", targeted = true, ignoreMoving = true }),
    touchOfTheMagi = NewSpell(321507, { damage = "magic", targeted = true, ignoreMoving = true }),
    arcaneMissles = NewSpell(5143, { damage = "magic", targeted = true, ignoreMoving = true }),
    arcaneBlast = NewSpell(30451, { damage = "magic", targeted = true, ignoreMoving = true }),
    arcaneBarrage = NewSpell(44425, { damage = "magic", targeted = true, ignoreMoving = true }),
    arcaneOrb = NewSpell(153626, { damage = "magic", targeted = false, ignoreMoving = true }),
    radiantSpark = NewSpell(376103, { damage = "magic", targeted = false, ignoreMoving = true }),
    pyroBlast = NewSpell(11366, { damage = "magic", targeted = true, ignoreMoving = false }),
    phoenixFlames = NewSpell(257541, { damage = "magic", targeted = true, ignoreMoving = true }),
    scorch = NewSpell(2948, { damage = "magic", targeted = true, ignoreMoving = false }),
    fireBall = NewSpell(133, { damage = "magic", targeted = true, ignoreMoving = false }),
    ...
    flamestrike = NewSpell(2120, { damage = "magic", diameter = 12, offsetMin = 0, offsetMax = 5.5, ignoreFacing = true }),
    ...
    presenceOfMind = NewSpell(205025, { beneficial = true, targeted = false, ignoreFacing = true }),
    netherPrecision = NewSpell(383783, { beneficial = true, targeted = false, ignoreFacing = true }),
    combustion = NewSpell(190319, { beneficial = true, targeted = false, ignoreFacing = true }),
    ...
    --## DEFENSIVE/UTILITY ##--
    blazingBarrier = NewSpell(235313, { beneficial = true, ignoreMoving = true, ignoreFacing = true }),
    ...
    massInvis = NewSpell(198158, { beneficial = true, ignoreMoving = true, ignoreFacing = true }),
    ...
    if select(2,IsInInstance()) == "arena" then
        awful.enemies.loop(function(enemy)
            if enemy.player then
                if target.buffFrom(enemyReflect) and player.castID ~= scorch.id then --stop casting on target if reflect
                    SpellStopCasting()
                end
                if enemy.buffFrom(enemyReflect) then -- remove reflect
                    if slow:Castable(enemy) then
                        slow:Cast(enemy)
                    end
                end
                if unit.hp > 15 and enemy.hp < 15 and not enemy.buffFrom(totalimmunity) and not enemy.debuffFrom(totalimmunity) and not enemy.bcc and enemy.los and not enemy.buffFrom(enemyReflect) then
                    if phoenixFlames:Castable(enemy) then 
                        phoenixFlames:Cast(enemy)
                    end
                    if fireBlast:Castable(enemy) then 
                        fireBlast:Cast(enemy)
                    end
                    if pyroBlast:Castable(enemy) and player.buff(48108) or player.buff(383874) then --48108 hot streak 383874 Hypertherima
                        pyroBlast:Cast(enemy) 
                    end
                    return true
                end
            end
...
function stomp()
    awful.totems.stomp(function(totem, uptime)
        if project.settings.stomps[totem.id] then
            ...
            if scorch:Castable(totem) and scorch.cd == 0 then
                scorch:Cast(totem)
            end
        end
    end)
...
--melee range indicator [Flamecannon]
Draw(function(draw)
    if project.settings.FlamecannonRangeCheck then
        local tx, ty, tz = player.position()
        draw:SetColorRaw(255, 255, 255, 0.3)
        draw:Outline(tx, ty, tz, 15)
    end
end)
...
fire:Init(function()
    ...
    if player.buff(massInvis.id) then return end --massInvis
    stomp()
    ...
    --blazingBarrier
    if not player.buff(blazingBarrier.id) then
        blazingBarrier:Cast() 
    end
    ...
    if project.settings.autoTargetToggle then
        if target.enemy then
            if not TargetCheck(target) and player.castID ~= scorch.id then
                SpellStopCasting()
                awful.enemies.loop(function(enemy) 
    ...
    if TargetCheck(target) and not player.buff(110960) and not player.buff(32612) and not player.buff(66) then --greater invis + normal invis(66+32612)
            ...
            if not enemy.dead then
                if enemy.player then
                    if enemy.speed > 25 then
                        if not enemy.buffFrom(enemyReflect) and not enemy.buffFrom(cantRootOrKnock) then
                            iceNova:Cast(enemy)
                        end
                    else
        ...
        --combustion
        if combustion:Castable() and target.distance < 40 and player.combat and project.settings.autoCombustion and project.settings.burstButton then
            combustion:Cast() 
        end

        --flamestrike
        if flamestrike:Castable(target) and target.distance < 40 and player.combat and enemies.around(target) > 2 then
            flamestrike:SmartAoE(target, { movePredTime = 1 })
        end

        --pyroBlast
        if pyroBlast:Castable(target) and (player.buff(48108) or player.buff(383874)) then --48108 hot streak 383874 Hypertherima
            pyroBlast:Cast(target) 
        end

        --fireBlast
        if fireBlast:Castable(target) and not (player.buff(48108) or player.buff(383874)) then
            fireBlast:Cast(target) 
        end

        --phoenixFlames
        if phoenixFlames:Castable(target) and not (player.buff(48108) or player.buff(383874)) then
            phoenixFlames:Cast(target) 
        end

        --fireBall
        if fireBall:Castable(target) and player.buff(203277) then -- 203277 flame accelerant
        ...
        --shiftingPower
        if ringOfFire.cd > 10 and combustion.cd > 10 and not player.moving and target.hp > 50 then
        ...
        if player.manaPct >= 50 then
            if project.settings.spellStealButton and not WasCasting[spellSteal.id] then
                awful.enemies.loop(function(enemy)  
                    if TargetCheck(enemy) then
        ...
        --scorch
        if scorch:Castable(target) then
            scorch:Cast(target) 
        end
    end
end)
